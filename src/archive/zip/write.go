...
func (z *ZipArchive) ExtractToFunc(destPath string, fn archive.HookFunc, entries ...string) (err error) {
	...
	for _, f := range z.File {
		...
		f.Name = archive.Clean(strings.ReplaceAll(f.Name, "\\", "/"))

		// Directory.
		if isDir {
			if isHasEntry {
				if archive.IsEntry(f.Name, entries) {
...
// specified destination.
// It accepts a function as a middleware for custom operations.
func ExtractToFunc(srcPath, destPath string, fn archive.HookFunc, entries ...string) (err error) {
	z, err := Open(srcPath)
	if err != nil {
		return err
	}
...
func (z *ZipArchive) extractFile(f *File) error {
	...
	return archive.Copy(f.tmpPath, f.absPath)
}
...
func packDir(srcPath string, recPath string, zw *zip.Writer, fn archive.HookFunc) error {
