...
func (tz *TzArchive) ExtractToFunc(destPath string, fn archive.HookFunc, entries ...string) (err error) {
	...
	for _, f := range tz.files {
		if !archive.IsExist(f.absPath) {
			continue
		}

		relPath := path.Join(destPath, f.Name)
		os.MkdirAll(path.Dir(relPath), os.ModePerm)
		if err := archive.Copy(relPath, f.absPath); err != nil {
			return err
		}
	}
	...
	for {
		...
		h.Name = archive.Clean(strings.ReplaceAll(h.Name, "\\", "/"))

		// Directory.
		if h.Typeflag == tar.TypeDir {
			if isHasEntry {
				if archive.IsEntry(h.Name, entries) {
...
func (tz *TzArchive) extractFile(f *File, tr *tar.Reader) error {
	...
	return archive.Copy(f.Name, f.absPath)
}
...
func (tz *TzArchive) Flush() (err error) {
	...
	tmpPath := filepath.Join(os.TempDir(), "archive", filepath.Base(tz.FileName))
	...
	for _, f := range tz.files {
		if strings.HasSuffix(f.Name, "/") {
			...
			continue
		} else if !archive.IsExist(f.absPath) {
			continue
		}
...
func packDir(srcPath string, recPath string, tw *tar.Writer, fn archive.HookFunc) error {
...
func packTo(srcPath, destPath string, fn archive.HookFunc, includeDir bool) error {
